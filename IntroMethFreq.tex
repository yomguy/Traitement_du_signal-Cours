...
\begin{equation}
 \boxeq{x(t) \cdot e^{2\pi f_0 t} \Leftrightarrow  X(f-f_0)}
\end{equation}
...
\begin{eqnarray}
 Y(f) & = & \int_{-\infty}^{+\infty} y(t)e^{-2j\pi f t} \ud t\\
 & = & \int_{-\infty}^{+\infty} x(at)e^{-2j\pi f t} \ud t \label{dem_ft_ech}
\end{eqnarray}
...
\begin{eqnarray}
 Y(f) & = & \int_{-\infty}^{+\infty} x(u)e^{-\frac{2j\pi f u}{a}} \frac{\ud u}{a}\\
 & = & \frac{1}{\vert a \vert} \int_{-\infty}^{+\infty} x(u) e^{-2j\pi \frac{f}{a}u} \ud u\\
